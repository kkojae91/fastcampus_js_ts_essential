...
var newObject1 = JSON.parse(JSON.stringify(sourceObject)); // 얕은 복사
...
console.log(sourceObject.traits.first_name.source_id); // 얕은 복사를 사용하면 2depth 부터는 참조하게 된다. 원본을 훼손하게 된다.

newObject2.traits.first_name.source_id = 100;
console.log(sourceObject.traits.first_name.source_id); // 얕은 복사를 사용하면 2depth 부터는 참조하게 된다. 원본을 훼손하게 된다.

newObject3.traits.first_name.source_id = 500;
console.log(sourceObject.traits.first_name.source_id); // data가 커지면 JSON.parse(JSON.stringify)의 성능이 좋지않다.
...
var newObject4 = deepCopyObject(sourceObject);
console.log(sourceObject.traits.first_name.source_id);
...
var newObject5 = _objectSpread(_objectSpread({}, deepCopyObject(store)), {}, {
  config: _objectSpread(_objectSpread({}, store.config), {}, {
    lastLoginData: new Date()
  })
});

console.log(newObject5); // default 값 설정하는 테크닉
...
function createParagraph(config) {
  config = _objectSpread(_objectSpread({}, DefaultStyle), config); // Do somethinf
